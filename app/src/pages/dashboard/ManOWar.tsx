...
export default function ManOWarDashboard() {
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Man O'War Instructor Dashboard</h1>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <QuickActionCard
                    ...
                    description="Submit Man O'War evaluations for your platoon"
                  ...
                  <TableBody>
                    {members?.map((member) => (
                      <TableRow key={member.id}>
                        ...
                        <TableCell>
                          <Badge variant={member.isEvaluatedByManOWar ? "default" : "secondary"} className={member.isEvaluatedByManOWar ? "bg-green-100 text-green-700" : ""}>
                            {member.isEvaluatedByManOWar ? "Evaluated" : "Pending"}
                          </Badge>
                        </TableCell>
                        <TableCell>
          ...
          <TabsContent value="print" className="mt-4">
            <Card>
              ...
              <CardContent>
                <p className="text-gray-600 mb-4">Generate a printable summary of all Man O'War evaluations for Platoon {platoon}.</p>
...
function EvaluationDialog({ member }: { member: any }) {
  ...
  const { data: existingEvaluation } = trpc.evaluations.getByCorpsMemberAndRole.useQuery(
    { corpsMemberId: member.id, evaluatorRole: "man_o_war_instructor" },
    { enabled: open }
  );

  const submitMutation = trpc.evaluations.submitManOWarEvaluation.useMutation({
    onSuccess: () => {
  ...
  const overallAverage = (Object.values(scores).reduce((a, b) => a + b, 0) / 8).toFixed(2);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" variant={member.isEvaluatedByManOWar ? "outline" : "default"} className={!member.isEvaluatedByManOWar ? "bg-[#004d00] hover:bg-[#003300]" : ""}>
          <Eye className="w-3 h-3 mr-1" />
          {member.isEvaluatedByManOWar ? "Update" : "Evaluate"}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Man O'War Evaluation: {member.surname} {member.otherNames}</DialogTitle>
        </DialogHeader>
